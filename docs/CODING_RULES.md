...
- 연결, 실행, Blockly, MAVLink 관련 변경은 구현 전 현재 런타임 경로와 실제 사용 계약 먼저 확인
- 구조 정리나 명칭 변경은 제거보다 기존 동작 유지와 역할 승계 가능성 먼저 검토
...
- 제외 요청되었거나 별도 예정인 연관 영역을 임의로 함께 수정하지 않음
...
- 분석이나 수정 전 관련 타입, 스토어, 서비스 계약을 함께 대조
- 작업 완료 후 문서 수정 필요 여부 확인, 필요 시 관련 active 문서 갱신

## 문제해결 우선순위
1. 실제 동작하는 해결책 우선
2. 현재 런타임 경로와 실제 사용 계약 확인
3. 기존 코드 패턴 분석 및 일관성 유지
4. 타입 안정성 보장
5. 테스트 가능한 구조 설계
6. 단, MAVLink 등 외부 결합이 강한 영역은 테스트 가능 구조 원칙의 무리한 강제 배제
...
- `control_repeat`, `control_for`, `control_if`, `control_if_else`, `control_while`은 기본 제어 블록으로 사용자 경로에서 지원함
