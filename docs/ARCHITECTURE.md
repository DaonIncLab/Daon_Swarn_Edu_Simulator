...
- MAVLink 경로는 GCS heartbeat(1Hz)와 별도로 failsafe watchdog(1Hz)을 실행한다.
  armed 상태였던 시스템에서 HEARTBEAT가 3초 이상 끊기면 자동 failsafe가 `NAV_LAND`를 먼저 전송하고
  이어서 `MISSION_CLEAR_ALL`를 best effort로 수행한다.
  failsafe 진입 시 미션 업로드/완료 대기 상태는 즉시 중단된다.
...
3. `Interpreter`가 `scenarioPlan`을 실행하며, 이동 명령에는 시나리오 속도 컨텍스트를 적용
   MAVLink 미션 경로에서는 `wait`를 timed loiter mission item으로 평탄화하고, `hover` 이후 진행 의미는 기체/오토파일럿 해석에 맡긴다
   또한 MAVLink 경로는 실행 트리를 먼저 command list로 평탄화한 뒤 배치 전송(`sendCommands(commands)`)을 기본으로 사용한다
   `while`/`until`은 런타임과 MAVLink 평탄화 경로 모두 기본 반복 상한(`maxIterations=1000`)을 가진다
