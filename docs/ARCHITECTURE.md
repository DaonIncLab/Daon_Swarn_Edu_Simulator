...
8. 연결 서비스가 Unity 임베드, 외부 WebSocket, MAVLink 시뮬레이션/실드론, 테스트 모드 중 하나로 명령 전달
...
- `UNITY_WEBGL`: 브라우저 안에 임베드된 Unity WebGL과 직접 통신
- `SIMULATION`: 외부 서버와 WebSocket 통신
- `MAVLINK_SIMULATION`: MAVLink 시뮬레이터 기반 모드
- `REAL_DRONE`: MAVLink UDP/Serial 기반 실제 드론 연결
...
즉, Unity 경로의 기본 모델은 “임베드 + JSON 브리지”. WebSocket 설명은 `SIMULATION` 또는 MAVLink 브리지 경로에만 적용.

### WebSocket and MAVLink
- `SIMULATION`은 외부 시뮬레이터와 WebSocket으로 연결
- `MAVLINK_SIMULATION`과 `REAL_DRONE`은 `MAVLinkConnectionService`가 처리
- 브리지 서버는 `server/bridge-server.mjs`에 있으며 `/mavlink` 경로 사용
...
3. 실행/시각화 계층이 드론 상태와 telemetry 소비
4. `useTelemetryStore`는 드론별 history를 쌓고 최대 개수 기준으로 pruning
