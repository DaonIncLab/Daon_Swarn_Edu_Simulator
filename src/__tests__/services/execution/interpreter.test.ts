...
describe("Interpreter", () => {
  ...
  beforeEach(() => {
    ...
    mockConnectionService = {
      sendCommands: vi
        .fn()
        .mockResolvedValue({ success: true } as CommandResponse),
  ...
  describe("Repeat Loop Execution", () => {
    ...
    test("flattens repeated commands into a single MAVLink batch", async () => {
      ...
      expect(sendCommandsSpy).toHaveBeenCalledWith([
        ...
        { action: CommandAction.WAIT, params: { duration: 2 } },
  ...
  describe("Wait Node Execution", () => {
    ...
    test("flattens wait nodes into MAVLink mission batches", async () => {
      const mavlinkService = new MAVLinkConnectionService();
      const sendCommandsSpy = vi
        .spyOn(mavlinkService, "sendCommands")
        .mockResolvedValue({ success: true, timestamp: Date.now() });
      ...
      expect(result.success).toBe(true);
      expect(sendCommandsSpy).toHaveBeenCalledWith([
        { action: CommandAction.TAKEOFF_ALL, params: { altitude: 5 } },
        { action: CommandAction.WAIT, params: { duration: 2 } },
        { action: CommandAction.LAND_ALL, params: {} },
      ]);
    });

    test("allows MAVLink command lists that continue after hover", async () => {
      const mavlinkService = new MAVLinkConnectionService();
      const sendCommandsSpy = vi
        .spyOn(mavlinkService, "sendCommands")
        .mockResolvedValue({ success: true, timestamp: Date.now() });
    ...
    test("keeps move missions at three items even with scenario speed context", async () => {
      const mavlinkService = new MAVLinkConnectionService();
      coordinateConverter.setHome(37.7749, -122.4194, 0);
      (mavlinkService as any).transport = {
        isOpen: () => true,
        sendPacket: vi.fn().mockResolvedValue(undefined),
      };
      (mavlinkService as any).missionPositionCache.set(1, {
        x: 0,
        y: 0,
        z: 0,
        heading: 0,
      ...
      vi.spyOn(
        mavlinkService as any,
        "_executeBufferedMission",
      ).mockImplementation(async function mockExecuteBufferedMission(
        this: any,
      ) {
      ...
      const sequenceNode: SequenceNode = {
        ...
        children: [
          ...
          },
          {
            id: "cmd_land",
            type: "command",
            command: { action: CommandAction.LAND, params: { droneId: 1 } },
          },
        ],
      };
      ...
      expect(capturedMissionItems.map((item) => item.command)).toEqual([
        MAV_CMD.NAV_TAKEOFF,
        MAV_CMD.NAV_WAYPOINT,
        MAV_CMD.NAV_LAND,
      ]);
      expect(
        capturedMissionItems.some(
          (item) => item.command === MAV_CMD.DO_CHANGE_SPEED,
        ),
      ).toBe(false);
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    test("execution continues after resetting from error state", async () => {
      ...
      mockConnectionService.sendCommands = vi
        .fn()
